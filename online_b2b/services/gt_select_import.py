...
def parse_headers(path: str) -> dict:
    ...
    if not so:
        return {
            "ok": False,
            "error": "Couldn't find the 'No.' (Sales Order) column — is this the "
...
def parse_lines(path: str) -> dict:
    ...
    if not doc or not item:
        return {
            "ok": False,
            "error": "Couldn't find 'Document No.' / 'No.' columns — is this the "
...
def preview(headers_path: str, lines_path: str) -> dict:
    ...
    for h in headers:
        ...
        if h["line_count"] == 0:
            warnings.append(f"Order {h['so_no']} has no matching lines in the Sales Lines file.")
    ...
    if orphan:
        warnings.append(
            f"{len(orphan)} line-SO(s) have no matching header — "
    ...
    try:
        with _conn() as (cur, d):
            ...
            cur.execute(
                ...
                (run_ts, MODE, src, 1, len(new_headers), total_lines, total_qty, total_val, "", ""),
            )
            ...
            cur.executemany(
                f"INSERT INTO order_headers ({', '.join(hdr_cols)}) VALUES ({hph})", hpayload
            )
            ...
            if lpayload:
                cur.executemany(
                    f"INSERT INTO order_lines ({', '.join(ln_cols)}) VALUES ({lph})", lpayload
                )
            cur.connection.commit()
        return {
            "ok": True,
            "run_id": run_id,
            "imported": len(new_headers),
