...
def upsert_code(channel: str, sku_code, item_no=None, ean=None, source: str = "manual") -> dict:
    ...
    with _conn() as (cur, d):
        ph = d["ph"]
        cur.execute(f"DELETE FROM {_TABLE} WHERE channel={ph} AND sku_code={ph}", (channel, sku))
        cur.execute(
            f"INSERT INTO {_TABLE} ({', '.join(_COLS)}) VALUES ({', '.join([ph] * len(_COLS))})",
            (channel, sku, _clean(ean) or None, _clean(item_no) or None, source, now),
        )
        cur.connection.commit()
    ...
    with _conn() as (cur, d):
        ...
        for item_no, sku, ean in src:
            if sku and str(sku).strip() and str(sku).lower() != "nan":
                rows.append(("Swiggy", _clean(sku), _clean(ean), _clean(item_no), "excel", now))
        cur.execute(
            f"DELETE FROM {_TABLE} WHERE channel='Swiggy' AND COALESCE(source,'excel') <> 'manual'"
        )
        if rows:
            cur.executemany(
...
def status() -> dict:
    ...
    try:
        ...
        with _conn() as (cur, d):
            cur.execute(
                f"SELECT channel, COUNT(*), MAX(updated_at) FROM {_TABLE} "
                f"GROUP BY channel ORDER BY channel"
            )
            by_channel = [
                {"channel": c, "count": int(n), "last_updated": u} for c, n, u in cur.fetchall()
            ]
            cur.execute(f"SELECT COUNT(*) FROM {_TABLE}")
...
def list_codes(channel: str = "", q: str = "", limit: int = 300) -> dict:
    ...
    try:
        with _conn() as (cur, d):
            ...
            total = int(cur.fetchone()[0] or 0)
            cur.execute(
                f"SELECT {', '.join(cols)} FROM {_TABLE} {wsql} "
                f"ORDER BY channel, sku_code LIMIT {int(limit)}",
                args,
            )
            rows = [dict(zip(cols, r)) for r in cur.fetchall()]
        return {"rows": rows, "total": total, "channels": channels, "channel": channel, "q": q}
    except Exception:  # noqa: BLE001
