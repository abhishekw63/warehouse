...
def marketplace_rules() -> list:
    ...
    try:
        ...
        for name, c in cfgs.items():
            ...
            parser = c.get("file_parser") or c.get("pdf_parser") or c.get("source_format") or ""
            ...
            if gmd is not None:
                gst_margin = [
                    {"gst": g, "pct": round((1 - gmd * (1 + g / 100.0)) * 100, 2)}
                    for g in (18, 12, 5, 0)
                ]
...
_ROLE_KEYS = [
    ("po_col", "PO", "po"),
    ("loc_col", "Destination", "loc"),
    ("ean_col", "Item · EAN", "item"),
    ("item_col", "Item", "item"),
    ("sku_col", "Item · SKU", "item"),
    ("qty_col", "Quantity", "qty"),
    ("fob_col", "Vendor cost", "cost"),
    ("mrp_col", "MRP", "mrp"),
    ("po_date_col", "PO date", "date"),
    ("exp_date_col", "Expiry", "exp"),
    ("hsn_col", "HSN", "hsn"),
    ("amount_col", "Amount", "amt"),
]
...
def marketplace_templates() -> dict[str, dict]:
    ...
    for name, s in samples.items():
        ...
        for col in s.get("columns", []):
            ...
            cols.append({"name": col, "role": role, "role_class": cls, "used": bool(role)})
        ...
        grid = [
            [
                {"value": r.get(c["name"], ""), "used": c["used"], "role_class": c["role_class"]}
                for c in cols
            ]
            for r in s.get("rows", [])
        ]
...
def location_rules() -> list:
    ...
    try:
        ...
        return sorted(
            ({"loc": k, "mkt": v} for k, v in LOCATION_MARKETPLACE.items()),
...
class Processor:
    ...
    def __init__(self, marketplace, po_paths, warehouse=None, margin_pct=None, ean_fixes=None):
    ...
    def use_multi(self, config) -> bool:
        ...
        return bool(
            config.get("source_format") == "pdf"
            or config.get("pdf_parser")
    ...
    def _run(self, skip_dedup=False):
        ...
        if mstore.table_count() == 0:
            return {
                "ok": False,
                "error": "Ship-To mapping DB is empty — seed it on the Ship-To Mapping page first.",
            }
        ...
        if loc_count == 0:
            return {
                "ok": False,
                "error": f"No mapping locations for "
        ...
        if iml.table_count() == 0:
            return {
                "ok": False,
                "error": "Item master DB is empty — upload it on the Item Master page first.",
            }
        try:
            master = iml.DBMasterLoader().load_from_db()
        except Exception as e:  # noqa: BLE001
            return {"ok": False, "error": f"Item master load failed: {type(e).__name__}: {e}"}
        ...
        if not result.rows:
            return {
                "ok": False,
                "error": "No valid rows extracted from the PO file(s).",
    ...
    def generate_d365(self, out_path, actions=None) -> dict:
        ...
        template = SOExporter._D365_TO_TEMPLATE if is_to else SOExporter._D365_SO_TEMPLATE
        ...
        try:
            ...
            return {"ok": False, "error": f"D365 export failed: {type(e).__name__}: {e}"}
        excluded = len(self.result.rows) - len(filtered.rows)
        return {
            "ok": True,
            "d365_path": str(out_path),
            "lines": len(filtered.rows),
            "excluded": excluded,
        }
    ...
    def _amountless_to(self) -> bool:
        ...
        cfg = self.config or {}
        return getattr(self.result, "output_type", "so") == "to" and not cfg.get("amount_col")
    ...
    def _headers(self):
        ...
        headers = order_rows_from_result(self.result, self.marketplace, self.warehouse, "")
    ...
    def _lines(self, run_id=None, output_file="", actions=None):
        ...
        rows = lines_store.build_lines(
            self.result, run_id=run_id, output_file=output_file, actions=actions, ean_fixes=combined
        )
    ...
    def preview(self) -> dict:
        ...
        return {
            "ok": True,
            "summary": self._summary(lines, headers),
            "headers": headers,
            "lines": lines,
            "affected": affected,
    ...
    def confirm(self, actions=None) -> dict:
        ...
        if output_path is None:
            return {"ok": False, "error": "Workbook export failed.", "warnings": self.warnings}
        lines = self._lines()
        out: dict = {
            "ok": True,
        ...
        try:
            ...
            rows = self._lines(run_id=out["run_id"], output_file=str(output_path), actions=actions)
            out["lines_recorded"] = lines_store.insert_lines(out["run_id"], rows).get("recorded", 0)
            ...
            if self._amountless_to():
                upd = lines_store.set_order_value(out["run_id"], self._to_value_by_po(rows))
            ...
            if dts and out.get("run_id"):
                out["dates_backfilled"] = lines_store.set_po_dates(out["run_id"], dts).get(
                    "updated", 0
                )
        except Exception as e:  # noqa: BLE001
            out["ok"] = False
...
class FlipkartProcessor(Processor):
    ...
    def _tracker_labels(self) -> dict:
        ...
        return {
            str(t.get("PO", "")).strip(): t.get("Market Place", "")
            for t in tracker
            if t.get("Market Place")
        }
...
class FlipkartTOProcessor(Processor):
    ...
    def _consignment_files(self) -> list:
        return [p for p in self.po_paths if self._CONSIGNMENT_RE.search(Path(p).name)]

    def _visibility_file(self):
        return next((p for p in self.po_paths if self._VISIBILITY_RE.search(Path(p).name)), None)
    ...
    def run_engine(self, engine, files, config):
        ...
        if cons:
            ...
            return engine.process_consignments(
                cons, config, margin_pct=self.margin_pct, visibility_report_path=vis
            )
...
class MeeshoTOProcessor(Processor):
    ...
    def run_engine(self, engine, files, config):
        ...
        return engine.process_consignments(
            csvs, config, margin_pct=self.margin_pct, visibility_report_path=None
        )
...
_PROCESSORS = {
    "Flipkart": FlipkartProcessor,
    "Flipkart-TO": FlipkartTOProcessor,
    "Meesho-TO": MeeshoTOProcessor,
    "Dmart": DmartProcessor,
    "Firstcry": FirstcryProcessor,
}


def processor_for(
    marketplace, po_paths, warehouse=None, margin_pct=None, ean_fixes=None
) -> Processor:
    cls = _PROCESSORS.get(marketplace, Processor)
    return cls(marketplace, po_paths, warehouse, margin_pct, ean_fixes)


def preview(marketplace: str, po_paths, warehouse=None, margin_pct=None, ean_fixes=None) -> dict:
    return processor_for(marketplace, po_paths, warehouse, margin_pct, ean_fixes).preview()


def confirm(
    marketplace: str, po_paths, warehouse=None, margin_pct=None, actions=None, ean_fixes=None
) -> dict:
    return processor_for(marketplace, po_paths, warehouse, margin_pct, ean_fixes).confirm(actions)
...
def generate_d365(
    ...
    return processor_for(marketplace, po_paths, warehouse, margin_pct, ean_fixes).generate_d365(
        out_path, actions
    )
