...
def parse(filepath: str) -> dict:
    ...
    try:
        ...
        if "so_no" not in col:
            return {
                "ok": False,
                "error": "Couldn't find the 'No.' (Sales Order number) column — "
...
def ensure_columns() -> None:
    ...
    with _conn() as (cur, d):
        if d["kind"] == "mysql":
            ...
            if cur.fetchone()[0] == 0:
                cur.execute("ALTER TABLE order_headers ADD COLUMN external_doc VARCHAR(100) NULL")
        else:
            cols = [r[1] for r in cur.execute("PRAGMA table_info(order_headers)").fetchall()]
            if "external_doc" not in cols:
                cur.execute("ALTER TABLE order_headers ADD COLUMN external_doc TEXT")
...
def do_import(filepath: str) -> dict:
    ...
    if not new_rows:
        return {"ok": True, "run_id": None, "imported": 0, "skipped": len(pv["rows"])}
    ...
    try:
        with _conn() as (cur, d):
            ...
            cur.executemany(f"INSERT INTO order_headers ({cols}) VALUES ({phs})", payload)
            cur.connection.commit()
        return {
            "ok": True,
            "run_id": run_id,
