...
def ensure_table() -> None:
    ...
    with _conn() as (cur, d):
        ...
        try:  # backfill source col on an older table
            cur.execute(f"ALTER TABLE {_MAP_TABLE} ADD COLUMN source VARCHAR(10) DEFAULT 'excel'")
...
def build_rows(xlsx_path: str):
    ...
    missing = [k for k in ("party", "del_location", "cust_no", "ship_to") if k not in cmap]
    if missing:
        return (
            [],
            {},
            [f"Mapping file missing columns: {', '.join(missing)}. Available: {list(df.columns)}"],
        )
    ...
    for _, r in df.iterrows():
        ...
        rows.append(
            {
                "party": party[:60],
                "del_location": loc[:500],
                "cust_no": _cust(r[cmap["cust_no"]])[:40],
                "ship_to": g(r, "ship_to")[:60],
                "name": g(r, "name")[:255],
                "address": g(r, "address")[:500],
                "address2": g(r, "address2")[:500],
                "postcode": g(r, "postcode")[:20],
                "city": g(r, "city")[:120],
            }
        )
...
def replace_mapping(rows: list) -> dict:
    ...
    with _conn() as (cur, d):
        ...
        cur.execute(f"DELETE FROM {_MAP_TABLE} WHERE COALESCE(source,'excel') <> 'manual'")
        cur.executemany(f"INSERT INTO {_MAP_TABLE} ({cols}) VALUES ({marks})", payload)
        cur.connection.commit()
...
def add_mapping(fields: dict) -> dict:
    ...
    with _conn() as (cur, d):
        ph = d["ph"]
        cur.execute(
            f"INSERT INTO {_MAP_TABLE} ({', '.join(cols)}) VALUES ({', '.join([ph] * len(cols))})",
            vals,
        )
...
def update_mapping(row_id, fields: dict) -> dict:
    ...
    with _conn() as (cur, d):
        ph = d["ph"]
        cur.execute(
            f"UPDATE {_MAP_TABLE} SET {', '.join(f'{c}={ph}' for c in sets)} WHERE id={ph}",
            vals + [int(row_id)],
        )
        n = cur.rowcount
        cur.connection.commit()
...
def get_mapping(row_id) -> dict | None:
    ...
    with _conn() as (cur, d):
        ph = d["ph"]
        cur.execute(f"SELECT {', '.join(cols)} FROM {_MAP_TABLE} WHERE id={ph}", (int(row_id),))
        r = cur.fetchone()
...
def status() -> dict:
    ...
    try:
        ...
        with _conn() as (cur, d):
            cur.execute(
                f"SELECT COUNT(*), COUNT(DISTINCT party), MAX(updated_at) FROM {_MAP_TABLE}"
            )
            n, parties, last = cur.fetchone()
            cur.execute(f"SELECT party, COUNT(*) FROM {_MAP_TABLE} GROUP BY party ORDER BY party")
            by_party = [{"party": p, "count": int(c)} for p, c in cur.fetchall()]
        return {
            "ok": True,
            "count": int(n or 0),
            "parties": int(parties or 0),
            "last_updated": last,
            "by_party": by_party,
        }
    except Exception as e:  # noqa: BLE001
...
def list_mappings(party: str = "", q: str = "", limit: int = 200) -> dict:
    ...
    q = (q or "").strip()
    cols = ["id", "party", "del_location", "cust_no", "ship_to", "city", "postcode", "source"]
    try:
        with _conn() as (cur, d):
            ph = d["ph"]
            ...
            where, args = [], []
            if party:
                where.append(f"party={ph}")
                args.append(party)
            if q:
                like = f"%{q}%"
                where.append(
                    f"(del_location LIKE {ph} OR ship_to LIKE {ph} OR "
            ...
            cur.execute(
                f"SELECT {', '.join(cols)} FROM {_MAP_TABLE} {wsql} "
                f"ORDER BY party, del_location LIMIT {int(limit)}",
                args,
            )
        ...
        return {"rows": [], "total": 0, "shown": 0, "party": party, "q": q, "parties": []}
...
class DBMappingLoader(MappingLoader):
    ...
    def load(self, filepath, party_name, logs) -> int:  # noqa: ARG002 — filepath unused
        ...
        with _conn() as (cur, d):
            cur.execute(f"SELECT party, del_location, cust_no, ship_to FROM {_MAP_TABLE}")
        ...
        for party, location, cust_no, ship_to in fetched:
            ...
            if location and location.lower() != "nan":
                ...
                if ship_to and ship_to.lower() != "nan":
                    self.by_shipto.setdefault(ship_to.upper(), {**entry, "matched_key": ship_to})
