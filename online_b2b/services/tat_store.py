...
def breaches(
    marketplace="", segment="", q="", status="pending", date_from="", date_to="", run="", limit=300
) -> dict:
    ...
    try:
        with _conn() as (cur, d):
            ...
            where = [
                "h.po_date IS NOT NULL",
                "h.run_ts IS NOT NULL",
                f"DATEDIFF(DATE(h.run_ts), h.po_date) >= {TAT_DAYS + 1}",
            ]
            params: list = []
            if marketplace:
                where.append(f"h.marketplace_label={ph}")
                params.append(marketplace)
            if segment:
                where.append(f"h.segment={ph}")
                params.append(segment)
            if q:
                where.append(f"(h.po LIKE {ph} OR h.location LIKE {ph})")
                params += [f"%{q}%", f"%{q}%"]
            if date_from:
                where.append(f"DATE(h.run_ts) >= {ph}")
                params.append(date_from)
            if date_to:
                where.append(f"DATE(h.run_ts) <= {ph}")
                params.append(date_to)
            if run:
                where.append(f"h.run_id={ph}")
                params.append(run)
            wsql = " AND ".join(where)
            cols = [
                "order_id",
            ...
            cur.execute(
                ...
                f"ORDER BY h.po_date ASC, h.order_id DESC",
                tuple(params),
            )
            pend = res = 0
            kept = []
