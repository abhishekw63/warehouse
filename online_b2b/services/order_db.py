...
def _tag_basis(rows: list[dict]) -> list[dict]:
    ...
    for r in rows:
        r["basis"] = (
            "CP"
            if r.get("vendor_cp") is not None
            else "Landing"
            if r.get("vendor_landing") is not None
            else ""
        )
    return rows
    ...
    try:
        with _conn() as (cur, d):
            ...
            if date:
                ...
                wsql, params = (f"created_at >= (CURDATE() - INTERVAL {int(days) - 1} DAY)", ())
            cur.execute(
                f"SELECT segment, marketplace, marketplace_label, COUNT(DISTINCT po), "
                f"COALESCE(SUM(order_value),0), COALESCE(SUM(items),0), "
                f"COALESCE(SUM(qty),0) "
                f"FROM {ot} WHERE {wsql} "
                f"GROUP BY segment, marketplace, marketplace_label",
                params,
            )
            tree: dict = {}
            tp = tv = ti = tq = 0
            for seg, mkt, label, pos, val, items, qty in cur.fetchall():
                ...
                m = s["mkts"].setdefault(
                    mkt, {"pos": 0, "value": 0.0, "items": 0, "qty": 0, "children": []}
                )
                m["children"].append(
                    {"label": label, "pos": pos, "value": round(val, 2), "items": items, "qty": qty}
                )
                m["pos"] += pos
                m["value"] += val
                m["items"] += items
...
def hub_extra_kpis() -> dict:
    ...
    try:
        with _conn() as (cur, d):
            ...
            cur.execute(
                f"SELECT COUNT(DISTINCT po) FROM {ot} "
            ...
            try:
                ...
                cur.execute(
                    "SELECT COUNT(*) FROM order_lines_full "
    ...
    try:
        with _conn() as (cur, d):
            ...
            for s, p, v in cur.fetchall():
                p = p or 0
                v = float(v or 0)
                segs.append({"segment": _label.get(s, s or "Other"), "pos": p, "value": v})
                tot_pos += p
                tot_val += v
    ...
    try:
        with _conn() as (cur, d):
            ...
            cur.execute(
                ...
                f"COALESCE(SUM(order_value),0) FROM {ot} WHERE {seg}",
                tuple(params),
            )
            r = cur.fetchone()
            out = {"pos": r[0] or 0, "qty": int(r[1] or 0), "value": float(r[2] or 0)}
    except Exception:  # noqa: BLE001
        pass
    return out
...
def _where(d: dict, f: dict):
    ...
    where = [seg_sql]
    if f["marketplace"]:
        where.append(f"marketplace_label={ph}")
        params.append(f["marketplace"])
    if f["days"] > 0:
        where.append(f"run_ts >= {ph}")
        params.append(_cutoff(kind, f["days"]))
    if f["q"]:
        where.append(f"po LIKE {ph}")
        params.append(f"%{f['q']}%")
    if f["warehouse"]:
        where.append(f"warehouse={ph}")
        params.append(f["warehouse"])
    if f["order_type"]:
        where.append(f"order_type={ph}")
        params.append(f["order_type"])
    if f["date_from"]:
        where.append(f"po_date >= {ph}")
...
def _fetch_orders(cur, d, f, sort="date", direction="desc", limit=PAGE_SIZE, offset=0):
...
def dashboard(
    ...
    try:
        with _conn() as (cur, d):
            ...
            for key, colname in (
                ("marketplace_options", "marketplace_label"),
                ("warehouse_options", "warehouse"),
                ("type_options", "order_type"),
            ):
                cur.execute(
                    f"SELECT DISTINCT {colname} FROM {ot} WHERE {seg} "
            ...
            out["orders"] = _fetch_orders(cur, d, f, sort, direction, limit, offset)
...
def orders_page(sort="date", direction="desc", offset=0, limit=PAGE_SIZE, **filters) -> dict:
...
def _kpis(cur, d, segment=SEGMENT) -> dict:
    ...
    cur.execute(
        f"SELECT COUNT(DISTINCT po) FROM {ot} WHERE {seg} AND run_ts >= {ph}",
        tuple(sp + [_cutoff(kind, RECENT_DAYS)]),
    )
    ...
    try:
        cur.execute(
            "SELECT COUNT(*), COUNT(DISTINCT po) FROM order_lines_full "
    ...
    k = {
        "pos": n_pos,
        "lines": n_lines,
        "qty": int(tot_qty or 0),
        "value": float(tot_val or 0.0),
        "marketplaces": n_mp,
        "updated_2d": updated_2d,
...
def _deltas(cur, d, segment=SEGMENT) -> dict:
    ...
    return {"pos_delta": pct(cur_pos, prev_pos), "value_delta": pct(cur_val, prev_val)}
...
def _trends(cur, d, days=30, segment=SEGMENT) -> dict:
    ...
    for i in range(days - 1, -1, -1):
        ...
        series.append(
            {"date": day.isoformat(), "label": day.strftime("%d %b"), "orders": c, "value": v}
        )
...
def _mp_sparklines(cur, d, days=30, segment=SEGMENT) -> dict[str, str]:
    ...
    for lbl, dmap in by.items():
        vals = [dmap.get(today - _dt.timedelta(days=i), 0.0) for i in range(days - 1, -1, -1)]
        mx = max(vals) or 1.0
        nn = len(vals)
        pts = [
            (round(i / (nn - 1) * W, 1), round(PAD + (1 - v / mx) * (H - 2 * PAD), 1))
...
_RESOLVED_SQL = (
    "((action IS NOT NULL AND action <> '') OR (received_ean IS NOT NULL AND received_ean <> ''))"
)
_PENDING_SQL = "(action IS NULL OR action = '') AND (received_ean IS NULL OR received_ean = '')"
...
_AFFECTED_SQL = (
    "(status IN ('MISMATCH','NOT_IN_MASTER') OR (received_ean IS NOT NULL AND received_ean <> ''))"
)
_FIXED_EAN_SQL = "(received_ean IS NOT NULL AND received_ean <> '')"


def issues(
    marketplace="", q="", status="", resolution="pending", date_from="", date_to="", limit=300
) -> dict:
    ...
    res_sql = {"pending": _PENDING_SQL, "resolved": _RESOLVED_SQL}.get(resolution, "1=1")
    out = {
        "ok": False,
        "rows": [],
        "counts": {},
    ...
    try:
        with _conn() as (cur, d):
            ...
            if status == "NOT_IN_MASTER":
                ...
                status_clause = f"status={ph}"
                params = [status]
            else:
                status_clause = _AFFECTED_SQL
                params = []
            where = [status_clause, res_sql]
            if marketplace:
                where.append(f"marketplace={ph}")
                params.append(marketplace)
            if q:
                where.append(f"(po LIKE {ph} OR item_no LIKE {ph} OR description LIKE {ph})")
                params += [f"%{q}%", f"%{q}%", f"%{q}%"]
            # Upload-date window (on run_ts). Same fragment reused for the counts.
            date_sql = ""
            date_params: list = []
            if date_from:
                date_sql += f" AND DATE(run_ts) >= {ph}"
            ...
            cur.execute(
                ...
                f"ORDER BY line_id DESC{lim_sql}",
                tuple(params) + tuple(date_params),
            )
            ...
            cur.execute(
                ...
                f"WHERE {_AFFECTED_SQL} AND {res_sql}{date_sql}",
                tuple(date_params),
            )
            mm, nim = cur.fetchone()
            ...
            cur.execute(
                ...
                f"SUM(CASE WHEN status='NOT_IN_MASTER' OR {_FIXED_EAN_SQL} "
                f"THEN 1 ELSE 0 END) FROM order_lines_full "
                f"WHERE {_AFFECTED_SQL}{date_sql}",
                tuple(date_params),
            )
            tmm, tnim = cur.fetchone()
            out["counts_total"] = {"MISMATCH": int(tmm or 0), "NOT_IN_MASTER": int(tnim or 0)}
            cur.execute(
                f"SELECT COUNT(*) FROM order_lines_full "
...
def sku_summary(
    marketplace="", q="", date_from="", date_to="", issues_only=False, limit=5000
) -> dict:
    ...
    try:
        with _conn() as (cur, d):
            ph = d["ph"]
            cur.execute("SELECT DISTINCT marketplace FROM order_lines_full ORDER BY marketplace")
            ...
            if q:
                where.append(f"(item_no LIKE {ph} OR ean LIKE {ph} OR description LIKE {ph})")
                args += [f"%{q}%", f"%{q}%", f"%{q}%"]
            if date_from:
                where.append(f"run_ts >= {ph}")
            ...
            for r in rows:
                ...
                ds = [x for x in (r.pop("min_diff"), r.pop("max_diff")) if x is not None]
...
def sku_analytics(
    date_from="", date_to="", marketplace="", top: int = 10, full: bool = False
) -> dict:
    ...
    try:
        with _conn() as (cur, d):
            ph = d["ph"]
            cur.execute(
                "SELECT DISTINCT marketplace FROM order_lines_full "
            ...
            cur.execute(
                ...
                f"FROM order_lines_full WHERE {wsql} GROUP BY item_no",
                tuple(args),
            )
            rows = _rows(cur, ["item_no", "description", "qty", "value", "pos", "lines", "mps"])
            ...
            cur.execute(
                f"SELECT COUNT(DISTINCT po) FROM order_lines_full WHERE {wsql}", tuple(args)
            )
...
def orders_for_export(sort="date", direction="desc", **filters) -> list[dict]:
    ...
    f = _norm_filters(filters)
    with _conn() as (cur, d):
        return _fetch_orders(cur, d, f, sort, direction, limit=1000000, offset=0)
...
def _lines_where(ph, marketplace, status, po, q):
    ...
    if po:
        where.append(f"po={ph}")
        params.append(po)
    if q:
        where.append(
            f"(po LIKE {ph} OR item_no LIKE {ph} OR ean LIKE {ph} OR description LIKE {ph})"
        )
        params += [f"%{q}%"] * 4
    return " AND ".join(where), params


def line_items(marketplace="", status="", po="", q="", offset=0, limit=PAGE_SIZE) -> dict:
    ...
    try:
        with _conn() as (cur, d):
            ...
            cur.execute(
                f"SELECT COUNT(*), COALESCE(SUM(qty),0), "
            ...
            cur.execute(
                "SELECT DISTINCT status FROM order_lines_full WHERE status <> '' ORDER BY status"
            )
...
def line_items_page(marketplace="", status="", po="", q="", offset=0, limit=PAGE_SIZE) -> dict:
    ...
    try:
        with _conn() as (cur, d):
            ph = d["ph"]
            wsql, params = _lines_where(ph, marketplace, status, po, q)
            cur.execute(f"SELECT COUNT(*) FROM order_lines_full WHERE {wsql}", tuple(params))
...
def run_detail(run_id: int) -> dict:
    ...
    out: dict = {"ok": False, "run": None, "orders": [], "lines": [], "issues": []}
    try:
        with _conn() as (cur, d):
            ot, ph = d["orders"], d["ph"]
            cur.execute(
            ...
            cur.execute(
                ...
                f"FROM {ot} WHERE run_id={ph} ORDER BY po",
                (run_id,),
            )
            out["orders"] = _rows(
                cur,
            ...
            try:
                ...
                out["lines"] = []
            out["issues"] = [
                l for l in out["lines"] if l["status"] in ("MISMATCH", "NOT_IN_MASTER")
            ]
        out["ok"] = True
    except Exception as e:  # noqa: BLE001
        out["error"] = f"{type(e).__name__}: {e}"
