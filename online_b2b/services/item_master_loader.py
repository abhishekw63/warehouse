...
def _upsert(cur, d, table, cols, values) -> None:
    ...
    if d["kind"] == "mysql":
        ...
        cur.execute(
            f"INSERT INTO {table} ({collist}) VALUES ({marks}) ON DUPLICATE KEY UPDATE {sets}",
            values,
        )
    else:
        cur.execute(f"INSERT OR REPLACE INTO {table} ({collist}) VALUES ({marks})", values)
...
def upsert_manual_item(data: dict) -> dict:
    ...
    if swiggy_code:
        ...
        channel_map.upsert_code(
            "Swiggy", swiggy_code, item_no=item_no, ean=rec["ean"], source="manual"
        )
...
def _read_effective_mrp(path: str, as_of: _dt.date):
    ...
    for item_no, grp in df.groupby("item_no"):
        ...
        covering = [
            r
            for r in rows
            if _has(r["start"]) and _has(r["end"]) and r["start"] <= as_of <= r["end"]
        ]
        if covering:
            ...
            if started:
                ...
                r = min(rows, key=lambda x: x["start"] if _has(x["start"]) else _dt.date.max)
                no_cover.append(item_no)
        if not _has(r["end"]):
...
def load_swiggy_map() -> dict:
    ...
    try:
        with _conn() as (cur, d):
            ph = d["ph"]
            cur.execute(
                f"SELECT item_no, sku_code FROM channel_sku_map WHERE channel={ph}", ("Swiggy",)
            )
            return {_clean(i): (s or "").strip() for i, s in cur.fetchall() if s and str(s).strip()}
...
def seed_swiggy_from_excel(master_xlsx: str) -> dict:
    ...
    try:
        ...
        sheet = next((s for s in xl.sheet_names if str(s).strip().lower() == "item master"), 0)
    ...
    with _conn() as (cur, d):
        ph = d["ph"]
        cur.execute(
            "DELETE FROM channel_sku_map WHERE channel='Swiggy' AND "
...
def replace_item_master(rows: list) -> dict:
    ...
    with _conn() as (cur, d):
        ...
        cur.execute(f"DELETE FROM {_MASTER_TABLE} WHERE COALESCE(batch_id,'') <> 'manual'")
        ...
        if d["kind"] == "mysql":
            ...
            sql = f"INSERT OR REPLACE INTO {_MASTER_TABLE} ({cols}) VALUES ({marks})"
        cur.executemany(sql, payload)
        cur.execute(f"SELECT COUNT(*) FROM {_MASTER_TABLE}")
        total = int(cur.fetchone()[0] or 0)
        cur.execute(f"SELECT COUNT(*) FROM {_MASTER_TABLE} WHERE COALESCE(batch_id,'')='manual'")
        manual = int(cur.fetchone()[0] or 0)
        cur.connection.commit()
    return {"ok": True, "rows": total, "batch_id": batch, "manual_overlaid": manual}
...
def status() -> dict:
    ...
    try:
        ...
        with _conn() as (cur, d):
            ...
            cur.execute("SELECT COUNT(*) FROM channel_sku_map WHERE channel='Swiggy'")
...
def list_items(q: str = "", limit: int = 100) -> dict:
    ...
    q = (q or "").strip()
    cols = ["item_no", "ean", "description", "mrp", "gst_code", "hsn", "mrp_start", "mrp_end"]
    try:
        with _conn() as (cur, d):
            ph = d["ph"]
            where, args = "", []
            if q:
                like = f"%{q}%"
                where = f"WHERE item_no LIKE {ph} OR ean LIKE {ph} OR description LIKE {ph}"
                args = [like, like, like]
            cur.execute(f"SELECT COUNT(*) FROM {_MASTER_TABLE} {where}", args)
            total = int(cur.fetchone()[0] or 0)
            cur.execute(
                f"SELECT {', '.join(cols)} FROM {_MASTER_TABLE} {where} "
                f"ORDER BY item_no LIMIT {int(limit)}",
                args,
            )
    ...
    try:
        with _conn() as (cur, d):
            ...
            cur.execute(
                ...
                f"{_MASTER_TABLE} WHERE ean={ph} OR item_no={ph} LIMIT 1",
                (k, k),
            )
            r = cur.fetchone()
            if r:
...
class DBMasterLoader(MasterLoader):
    ...
    def load_from_db(self, overlay_master_path: str | None = None):
        ...
        with _conn() as (cur, d):
            cur.execute(
                f"SELECT item_no, ean, description, gst_code, hsn, mrp FROM {_MASTER_TABLE}"
            )
...
def diff_against_current(rows: list) -> dict:
    ...
    try:
        ...
        with _conn() as (cur, _d):
            cur.execute(
                f"SELECT item_no, mrp, description, COALESCE(batch_id,'') FROM {_MASTER_TABLE}"
            )
        ...
        return {
            ...
            "mrp_changed": [],
            "removed": [],
            "counts": {"new": 0, "mrp_changed": 0, "removed": 0, "unchanged": 0},
            "any": False,
        }
    ...
    for r in rows:
        ...
        if cur_row is None:
            new.append(
                {"item_no": ino, "new_mrp": nm, "description": (r.get("description") or "")[:80]}
            )
        elif cur_row["mrp"] != nm:
            changed.append(
                {
    ...
    removed = [
        {"item_no": k, "old_mrp": v["mrp"], "description": v["description"][:80]}
        for k, v in cur_map.items()
        if k not in seen and not v["manual"]
    ]
    unchanged = max(0, len(seen) - len(new) - len(changed))
    counts = {
        "new": len(new),
        "mrp_changed": len(changed),
        "removed": len(removed),
        "unchanged": unchanged,
    }
    return {
