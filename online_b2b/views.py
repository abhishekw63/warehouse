...
class RulesView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            ctx["swiggy_deals"] = [
                r for r in overrides_store.list_all() if r.get("kind") == "swiggy_deal"
            ]
...
def sku_demand_export(request):
    ...
    for r in data.get("rows", []):
        w.writerow([r["item_no"], r["description"], r["qty"], r["value"], r["pos"], r["mps"]])
...
def _save_run_index(run_id, payload: dict) -> None:
    ...
    (_RUNS_INDEX / f"{run_id}.json").write_text(json.dumps(payload), encoding="utf-8")
...
def _persist_d365(run_id, marketplace, paths, warehouse, margin_pct, actions, ean_fixes=None):
...
def dashboard(request):
    ...
    return render(request, "online_b2b/overview.html", {"d": data, "branch": ONLINE_BRANCH})
...
def orders_more(request):
    ...
    return render(request, "online_b2b/_orders_rows.html", {"orders": page["orders"], "page": page})
...
def lines(request):
    ...
    data = order_db.line_items(offset=_int(request, "offset"), **_line_filters(request))
    if _is_ajax(request):
...
def lines_more(request):
    ...
    page = order_db.line_items_page(offset=_int(request, "offset"), **_line_filters(request))
    return render(request, "online_b2b/_lines_rows.html", {"rows": page["rows"]})
...
def issues(request):
    ...
    return render(
        request, "online_b2b/issues.html", {"d": data, "eanfix": order_db.ean_corrections()}
    )
...
def issues_export(request):
    ...
    cols = [
        ...
        ("item_no", "Item No"),
        ("ean", "EAN"),
        ("received_ean", "Received EAN"),
        ("description", "Description"),
        ("qty", "Qty"),
        ("vendor_mrp", "Vendor MRP"),
        ("our_mrp", "Our MRP"),
        ("vendor_cp", "Vendor CP"),
        ("our_cp", "Our CP"),
        ("vendor_landing", "Vendor Landing"),
...
def issues_email_send(request):
    ...
    if not rep.rows:
        return JsonResponse(
            {"ok": False, "error": "No issue lines in the current filter — nothing to send."}
        )
    ok, reason = rep.send()
...
def issues_save(request):
    ...
    res = lines_store.update_action(
        request.POST.get("line_id"), request.POST.get("action", ""), request.POST.get("remark", "")
    )
...
def issues_fix_ean(request):
    ...
    res = lines_store.apply_issue_ean_fix(
        request.POST.get("line_id"), (request.POST.get("correct_ean", "") or "").strip()
    )
...
def tat_export(request):
    ...
    cols = [
        ...
        ("qty", "Qty"),
        ("order_value", "Value"),
        ("wd_late", "Working days taken"),
        ("days_over", "Days over TAT"),
        ("reason_code", "Reason"),
        ("note", "Note"),
        ("reason_by", "By"),
        ("reason_at", "Reason at"),
    ...
    for r, row in enumerate(rows, 2):
        for c, (k, _h) in enumerate(cols, 1):
            v = row.get(k)
            ws.cell(
                r, c, str(v) if k in ("run_ts", "po_date", "reason_at") and v is not None else v
            )
    for col in ws.columns:
        L = col[0].column_letter
        w = max((len(str(c.value or "")) for c in col), default=8)
...
def sku_summary_lines(request):
    ...
    data = order_db.sku_lines(
        request.GET.get("item_no", "").strip(), request.GET.get("ean", "").strip()
    )
    ...
    key = json.dumps(
        {
            ...
            "wh": meta.get("warehouse"),
            "mg": meta.get("margin_pct"),
            "files": sorted(meta.get("files") or []),
...
def _cached_preview(token, d, meta, force=False):
    ...
    if res.get("ok"):
        try:
            cache.write_text(json.dumps({"sig": sig, "res": res}, default=str), encoding="utf-8")
...
def upload(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if form.is_valid():
                ...
                margin = form.cleaned_data.get("margin_pct") or engine_bridge.default_margin_pct(
                    marketplace
                )
                meta = {
                    "marketplace": marketplace,
                ...
                if _is_ajax(request):
                    ...
                    if not res.get("ok"):
                        return JsonResponse(
                            {"ok": False, "error": res.get("error", "Import failed.")}, status=200
                        )
                    s = res.get("summary", {})
                    return JsonResponse(
                        {
                            "ok": True,
                            "review_url": reverse("b2b_review", args=[token]),
                            "pos": s.get("pos", 0),
                            "lines": s.get("lines", 0),
                ...
                return JsonResponse({"ok": False, "error": errs or "Invalid form."}, status=200)
            ...
            if _is_ajax(request):
                return JsonResponse(
                    {"ok": False, "error": f"Upload crashed: {type(e).__name__}: {e}"}, status=200
                )
            raise
    else:
        form = UploadForm()
    return render(
        request,
        "online_b2b/upload.html",
        {"form": form, "margin_defaults": json.dumps(engine_bridge.margin_defaults())},
    )
...
def review(request, token):
    ...
    nim_lines = [ln for ln in (res.get("affected") or []) if ln.get("status") == "NOT_IN_MASTER"]
    ...
    exc_count = sum(
        1 for ln in (res.get("lines") or []) if (ln.get("exception_label") or "").strip()
    )
    return render(
        request,
        "online_b2b/review.html",
        {
            "token": token,
            "meta": meta,
            "r": res,
...
def confirm(request, token):
    ...
    if run_id is None:
        ...
        if ajax:
            ...
            return JsonResponse(
                {"ok": True, "run_id": None, "message": msg, "redirect": reverse("b2b_dashboard")}
            )
    ...
    messages.success(
        request, f"🔒 Locked & recorded {pos} PO(s), {lines} line(s). Now generate the D365 dump."
    )
    return redirect("b2b_review", token=token)
...
def fix_ean(request, token):
    ...
    for i, w in enumerate(wrongs):
        ...
        if not hit:
            errors.append(f"'{c}' is not in the item master — can't use it to correct EAN {w}.")
            continue
    ...
    if applied:
        ...
        (d / "meta.json").write_text(json.dumps(meta), encoding="utf-8")
        messages.success(
            request, f"✓ Corrected {applied} EAN(s) — re-validating against the master."
        )
    for e in errors:
...
def _full_workbook(outdir: Path):
    ...
    if not outdir.exists():
        return None
    files = sorted(
        [p for p in outdir.glob("*.xlsx") if not p.stem.endswith("_d365")],
...
def review_download(request, token):
    ...
    return FileResponse(open(f, "rb"), as_attachment=True, filename=_full_name(f.name))
...
def bulk_review(request, token):
    ...
    return render(request, "online_b2b/bulk_review.html", {"token": token, "r": res})
...
def bulk_confirm(request, token):
    ...
    messages.success(
        request, f"Imported {res['imported']} order(s) ({res.get('skipped', 0)} already present)."
    )
    return redirect("b2b_orders")
...
def download_d365(request, run_id):
    ...
    return FileResponse(open(path, "rb"), as_attachment=True, filename=f"{mkt}_D365_import.xlsx")
...
def download(request, run_id):
    ...
    return FileResponse(
        open(path, "rb"), as_attachment=True, filename=_full_name(os.path.basename(path))
    )
...
def item_master_upload(request):
    ...
    if not items_f or not mrp_f:
        messages.error(request, "Choose BOTH files — the Items export and the Item M.R.P. export.")
    ...
    (d / "meta.json").write_text(
        json.dumps(
            {
                "items_name": items_f.name,
                "mrp_name": mrp_f.name,
                "items_path": items_path.name,
                "mrp_path": mrp_path.name,
            }
        ),
        encoding="utf-8",
    )
...
def item_master_preview(request, token):
    ...
    return render(
        request,
        "online_b2b/item_master_preview.html",
        {
            "token": token,
            "meta": meta,
            "stats": stats,
...
def item_master_confirm(request, token):
    ...
    try:
        ...
        rows, stats, _ = iml.build_rows(str(d / meta["items_path"]), str(d / meta["mrp_path"]))
    ...
    messages.success(
        request,
        f"✓ Item master rebuilt — {res['rows']} items now live "
...
def _im_row_json(r: dict) -> dict:
    ...
    return {
        "item_no": s(r.get("item_no")),
        "ean": s(r.get("ean")),
        "description": s(r.get("description")),
        "gst_code": s(r.get("gst_code")),
        "hsn": s(r.get("hsn")),
        "mrp": None if r.get("mrp") is None else float(r["mrp"]),
        "mrp_start": s(r.get("mrp_start")),
        "mrp_end": s(r.get("mrp_end")),
    }
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["parents"] = [
            ...
            },
            {
                ...
                "tag": "live",
                "url": reverse("index"),
                "desc": "General trade mass — dump processing.",
                "children": [],
            },
            {
                "key": "GT Select",
                ...
                "desc": "D365-finalised — import Sales Orders + Lines.",
                "children": [],
            },
            {"key": "EKA", "name": "EKA", "tag": "soon", "desc": "Coming soon.", "children": []},
            {"key": "CSD", "name": "CSD", "tag": "soon", "desc": "Coming soon.", "children": []},
        ]
        return ctx
...
def gt_select_upload(request):
    ...
    if not hf or not lf:
        messages.error(request, "Choose BOTH files — Sales Orders (headers) and Sales Lines.")
    ...
    (d / "meta.json").write_text(
        json.dumps(
            {
                "headers_name": hf.name,
                "lines_name": lf.name,
                "headers_path": hp.name,
                "lines_path": lp.name,
            }
        ),
        encoding="utf-8",
...
def gt_select_preview(request, token):
    ...
    return render(
        request,
        "online_b2b/gt_select_preview.html",
        {
            "token": token,
            "meta": meta,
            "r": pv,
...
def gt_select_confirm(request, token):
    ...
    messages.success(
        request,
        f"✓ Imported {res['imported']} GT Select order(s) · "
...
def item_master_add(request):
    ...
    if not res.get("ok"):
        ...
        messages.success(
            request,
            f"✓ Item {res['item_no']} saved to the master "
...
class ShipToView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["overview"] = ms.list_mappings(
            self.request.GET.get("party", ""), self.request.GET.get("q", ""), limit=300
        )
        return ctx
...
def ship_to_upload(request):
    ...
    with open(dest, "wb") as out:
        for chunk in f.chunks():
            out.write(chunk)
    (d / "meta.json").write_text(json.dumps({"name": f.name, "path": dest.name}), encoding="utf-8")
...
def ship_to_preview(request, token):
    ...
    return render(
        request,
        "online_b2b/ship_to_preview.html",
        {
            "token": token,
            "meta": meta,
            "stats": stats,
...
def ship_to_confirm(request, token):
    ...
    messages.success(
        request,
        f"✓ Ship-To mapping replaced — {res['rows']} rows "
...
def ship_to_seed(request):
    ...
    if res.get("ok"):
        messages.success(
            request, f"✓ Seeded {res['rows']} rows from the bundled Ship-To B2B Excel."
        )
    else:
...
def ship_to_search(request):
    ...
    data = ms.list_mappings(request.GET.get("party", ""), request.GET.get("q", ""), limit=300)
