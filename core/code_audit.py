...
EXCLUDE = ('migrations', '__pycache__', '.venv', 'online_po_management',
           'standalone_files', 'node_modules', 'staticfiles')
...
def _ruff() -> dict:
    ...
    try:
        r = subprocess.run([str(exe), 'check', *DIRS, '--output-format', 'json'],
        ...
        findings = [{
            ...
            'code': d.get('code') or '', 'msg': d.get('message') or '',
        } for d in data]
        by_rule: dict = {}
        for f in findings:
            by_rule[f['code']] = by_rule.get(f['code'], 0) + 1
        top = sorted(by_rule.items(), key=lambda x: -x[1])[:12]
        return {'available': True, 'count': len(findings),
                'by_rule': top, 'findings': findings[:400]}
...
def _metrics() -> dict:
    ...
    for p in _py_files():
        ...
        total_lines += lines
        rel = _rel(p)
        todos += sum(1 for ln in src.splitlines()
                     if 'TODO' in ln or 'FIXME' in ln or 'XXX' in ln)
        if lines > 600:
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                if span > 80:
                    big_funcs.append({'file': rel, 'func': node.name,
                                      'lines': span, 'line': node.lineno})
    big_files.sort(key=lambda x: -x['lines'])
    big_funcs.sort(key=lambda x: -x['lines'])
    return {'files': nfiles, 'total_lines': total_lines, 'todos': todos,
            'big_files': big_files[:30], 'big_funcs': big_funcs[:30]}
...
def _duplication() -> list:
    ...
    groups = [v for v in seen.values()
              if len({loc.rsplit(':', 1)[0] for loc in v['locs']}) >= 2
              or len(v['locs']) >= 3]
    groups.sort(key=lambda v: -len(v['locs']))
    return [{'count': len(g['locs']), 'sample': g['sample'],
             'locations': g['locs'][:6]} for g in groups[:25]]
    ...
    for p in _py_files():
        ...
        for i, ln in enumerate(lines, 1):
            ...
            for rx, label, sev in _SEC_PATTERNS:
                if rx.search(ln):
                    hits.append({'file': rel, 'line': i, 'sev': sev,
                                 'issue': label, 'code': ln.strip()[:120]})
...
def run_audit() -> dict:
    res = {
        ...
        'ruff': _ruff(), 'metrics': _metrics(), 'duplication': _duplication(),
        'security': _security(),
    }
    try:
